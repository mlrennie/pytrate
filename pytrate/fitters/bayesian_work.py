...
class BayesianFitter(Fitter):
    ...
    def ln_prior(self,param):
        ...
        for i in range(n_params):
            if(self._dist_types[i]==1):
                ...
                err = "Separate errors for each concentration point has not be implemented yet"
                raise NotImplementedError(err)
    ...
    def ln_prob_obserr(self,param):
        ...
        # Calculate prior.  If not finite, this solution has an -infinity log 
        # likelihood
        ln_prior = self.ln_prior(param[:-1])
        if not np.isfinite(ln_prior):
            return -np.inf
        
        # Calcualte likelihood.  If not finite, this solution has an -infinity
        # log likelihood
        ln_like = self.ln_like_obserr(param)
        if not np.isfinite(ln_like):
            return -np.inf
    ...
    def fit(self,model,parameters,dist_types,dist_vars,y_obs,y_err,param_names=None):
        """
        Fit the parameters.       
        ...
        if(self._y_err.min() <= 0.):
            # assume a small error - update to educated guess based on range of data
            init_err = 1e-3
            self._initial_guess = np.append(self._initial_guess,init_err)
            ...
            self._fit_result = emcee.EnsembleSampler(self._num_walkers, ndim, self.ln_prob_obserr,
